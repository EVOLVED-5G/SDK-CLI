...
First, the partner to be interested in developing a NetApp must send an email to approval@evolved-5g.com to receive access to the GitHub organization.
...
First of all, in order to be able to work with Cookiecutter, it is mandatory to install it on your local computer or virtual machine. Below are the commands that should be executed to work with Cookiecutter.

This guide has been developed to work with Ubuntu OS, so all the commands have been tested under Ubuntu and Windows, if other OS is used, please have in mind some of this command may differ.
